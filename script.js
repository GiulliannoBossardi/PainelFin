...
function navigateTo(tab, skipSave=false){
  ...
  /* Sincroniza filtros visuais ao mudar aba */
  syncFiltroDisplays();
  /* Atualiza renderizações dependentes do filtro global */
...
/* ════════════════════════════════════════════
   ...
   filtroAno e filtroRef são globais e usados por todas as abas.
   Cada aba exibe um clone sincronizado dos selects.
════════════════════════════════════════════ */
let filtroAno='', filtroRef='';

/* IDs dos selects "espelho" em cada aba */
...
function onFiltroAnoChange(){
  /* Lê de qualquer um dos selects (todos sincronizados) */
...
function expandirSaidas(saidas,ano,ref){
  ...
  saidas.forEach(s=>{
    if(s.forma==='avista'){
      const r=s.ref;
      const ok=ref?r===ref:(ano?r.startsWith(ano+'-'):true);
      if(ok)rows.push({...s,parcelaNum:null,parcelaRef:r,parcelaTotal:1,valorExib:s.valor,_parcelaIdx:'av'});
    }else{
      for(let i=0;i<(s.nParcelas||1);i++){
        const pr=Fmt.addMonths(s.primeiraParcela,i);
        const ok=ref?pr===ref:(ano?pr.startsWith(ano+'-'):true);
        if(ok)rows.push({...s,parcelaNum:i+1,parcelaRef:pr,parcelaTotal:s.nParcelas,valorExib:s.valor,_parcelaIdx:i});
      }
    }
  });
  return rows;
...
/* ════════════════════════════════════════════
   ABA CONTROLE
   Receitas = Salário Líquido + Extras Líquido + Entradas
   Despesas = Saídas
   Investimentos = aportes na aba Investimento
...
function getLinhasCtrl(ano,ref){
  ...
  salRows.forEach(r=>{
    if(r.liquido>0)linhas.push({ref:r.ref,origem:'Salário',tipo:'Salário Líquido',descricao:'Adiantamento + Pagamento',valor:r.liquido,natureza:'receita'});
  });
...
function init(){
  ...
  /* Campos de data iniciam no mês atual */
  const ym=Fmt.nowYM();
  ...
  /* FILTRO GLOBAL inicia no mês atual */
  filtroAno=ym.split('-')[0];
  filtroRef=ym;

  /* Constrói opções de filtro (usa todos os dados + o mês atual para garantir) */
  reconstruirFiltros();
  ...
  /* Verifica sessão — se existir, entra direto sem mostrar login */
